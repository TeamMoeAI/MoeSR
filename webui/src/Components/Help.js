...
function Help() {
    return (
        <div className="HelpContainer">
            ...
            <strong>English help is at the bottom (language settings will be added in the future)</strong>
            <p>1. 模型比较</p>
            <p>当前版本为预览版本，可能不稳定。由于需要时间训练与测试模型，模型发布会晚于软件本体发布，选项中缺少下方模型是正常的。</p>
            <figure><table>
                <thead>
                    <tr><th>模型名称</th><th>描述</th></tr></thead>
                <tbody>
                    <tr><td>Real-ESRGAN-x4: Anime6B-Official</td><td>RealESRGAN官方提供的动画插画模型</td></tr>
                    <tr><td>Real-ESRGAN-x4: jp_Illustration-fix1</td><td>适用于日系插画，一般修复强度（去模糊，JPEG还原），模型适当自由发挥</td></tr>
                    <tr><td>Real-ESRGAN-x4: jp_Illustration-fix1-d</td><td>适用于日系插画，一般修复强度，保留更多细节</td></tr>
            ...
            <figure><table>
                ...
                <tbody>
                    <tr><td>Model</td><td>选择你需要的模型</td></tr>
                    <tr><td>Tile Size</td><td>分块大小，越大占用显存越高，但处理更快一些</td></tr>
                    <tr><td>Scale</td><td>需要放大的倍数，除仅修复模型外，如果该值与模型放大倍数不同，小于模型倍数则图片将会被缩放，大于模型倍数则图片会被重复超分辨率处理并缩放到目标倍数</td></tr>
                    <tr><td>Skip Alpha</td><td>勾选后，透明图片则不使用超分辨率模型处理透明通道，而采用插值处理</td></tr>
                    <tr><td>Resize To</td><td>缩放图片到指定尺寸。格式：长x宽；或者分数，例如1/2。输入时注意：乘号为小写半角英文x，分数为英文斜杠。（如果同时指定缩放倍率与该值，缩放倍率导致的超分次数不变，但最终图片尺寸会被缩放为该值）</td></tr>
                </tbody>
            </table></figure>
            <p>3. 处理参数</p>
            <figure><table>
                <thead>
                    <tr><th>参数名</th><th>描述</th></tr></thead>
                <tbody>
                    <tr><td>Input Image(Folder)</td><td>待进行超分辨率的图片(文件夹)，点击旁边“Select”按钮选择，仅支持jpg/png</td></tr>
                    <tr><td>Save To</td><td>保存图片的位置</td></tr>
                    <tr><td>Progress</td><td>单张图片进度。未来会加上剩余时间与总进度</td></tr>
                    <tr><td>Batch Process </td><td>批处理，勾选后可以选择整个文件夹作为输入</td></tr>
            ...
            <p>1. Model Compare</p>
            <p>The current version is a preview version and may be unstable. Due to the time required for training and testing models, the model release may be later than the software release, so it is normal for the model options to be missing below.</p>
            <figure>
            <table>
            <thead>
                ...
                <tbody>
                    ...
                    <tr><td>Real-ESRGAN-x4: jp_Illustration-fix1</td><td>Suitable for Japanese Style illustration, general restoration strength (de-blurring, JPEG restoration)</td></tr>
                    <tr><td>Real-ESRGAN-x4: jp_Illustration-fix1-d</td><td>Suitable for Japanese Style illustrations, general restoration strength, more details retained</td></tr>
